    ...
    <div class="col-md-12">
        ...
        <div class="panel panel-white">
            ...
            <div class="panel-body">
                <h2><i class="fa fa-pencil-square"></i> Verificacion de Inventario por Persona</h2>
                <p>
                    Esta es la seccion de Ingreso de Activos Fijos
                </p>
                <hr>

                <form:form method="POST" action="${pageContext.request.contextPath}/Descargo/insertarDescargo"  modelAttribute="verificarinv_persona" id="descargoF" onsubmit="return valida_envioDescargo();" >
                    <div class="row">
                        <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                             ...
                             <div class="form-group">
                                <label for="form-field-select-3">
                                   Persona<span id="span_clasi" class="symbol "></span>
                                </label>

                                <form:select path="" class="form-control search-select" id="Persona" name="Persona" >
                                    <form:option value="0"  label="Seleccione una Persona"/>
                                    <c:forEach var="pers" items="${persona}">
                                        <form:option value="${pers.idPersona}"  label="${pers.nombrePersona}"/>
                                    </c:forEach>
                                </form:select>
                                <span for="clasiL" class="help-block  no-display" id="span_dropdownT">Seleccione una Persona</span>
                            </div>
       ...
       //alert($("#ingresar").val());
        ...
        if ($('#VerificarInv tr').size() > 1)        
        {
            ...
            $('#VerificarInv tr').each(function (index, element) {
                ...
                var Persona = $(element).find("td").eq(3).html();

                if (l != 0)
                {
                    jsonArray = jsonArray + "{\"idInv\":" + '"' + id + '"'+",\"codigo\":"+'"'+codigo+'"'+",\"idlocalizacion\":"+'"'+Persona+'"'+"},";
                }
                l = 1;
            });
            ...
            $.ajax({
                ...
                type: "POST",
                url: "${pageContext.request.contextPath}/VerificarInventario/add/agregarTBTemporalPersona",
                contentType: 'application/json',
                dataType: "json",
                data: jsonArray,
